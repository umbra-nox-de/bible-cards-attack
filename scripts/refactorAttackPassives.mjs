...
const triggerPattern=/  if\(active==='Matthew'&&battleRng\.chance\(0\.25\)\)\{[\s\S]*?  if\(active==='Elijah'\)setEnemyStatus\(e=>\(\{\.\.\.e,burn:3\}\)\);/;
...
if(!text.includes("const attackTriggers=resolveAttackTriggers")){
  if(!triggerPattern.test(text))throw new Error("Attack trigger anchor not found.");
  text=text.replace(triggerPattern,newTriggers);
}
